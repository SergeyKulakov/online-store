// some helpers to limit boilerplate and control types for actions and reducers
// mostly some thought experiments to see if we can leverage typescript
...
export const actionTypeGenerator = (prefix: string) => {
  return {
    async: function asyncTypeGenerator(action: string): AsyncActionType {
      return {
...
export function asyncReducer<T, K extends keyof T>(
  actionType: AsyncActionType, key: K, override?: {
    loading?: ActionReducer<T>;
    success?: ActionReducer<T>;
    error?: ActionReducer<T>;
  }):
...
export const mappedLoadingReducer = <T, K extends keyof T>(key: K) => {
  return (store: T, action: ActionWithId) => ({
    ...store,
    [key]: {
      ...store[key],
      [action.id]: loadingAPIResult()
    }
  });
};

export const mappedSuccessReducer = <T, K extends keyof T, V>(key: K) => {
  return (store: T, action: SuccessAction<V> & ActionWithId) => {
    if (isSuccessAction(action)) {
      return {
        ...store,
        [key]: {
          ...store[key],
          [action.id]: successAPIResult(action.value)
        }
      };
    }
    return store;
...
export const mappedErrorReducer = <T, K extends keyof T, E>(key: K) => {
  return (store: T, action: ErrorAction<E> & ActionWithId) => ({
    ...store,
    [key]: {
      ...store[key],
      [action.id]: errorAPIResult(action.error)
    }
  });
};

export function mappedAsyncReducer<T, K extends keyof T>(
  actionType: AsyncActionType, key: K, override?: {
    loading?: any;
    success?: any;
    error?: any;
...
AsyncReducerResult<T> {
  return [
    [actionType.start, (override && override.loading) || mappedLoadingReducer(key)],
    [actionType.done, (override && override.success) || mappedSuccessReducer(key)],
    [actionType.fail, (override && override.error) || mappedErrorReducer(key)]
  ];
}

export const persistentLoading = <T, K extends keyof T>(key?: K) => {
  return (store: T): T => {
    key = !key ? ('data' as K) : key;
    return {
      ...store,
      [key]: {
        ...store[key],
